# DONE — closed work archive

**Archived 2026-09-06.** This was `TODO.md` until every ticket in it carried a
closed marker. It is kept in full for its reasoning, provenance and rejected
premises, which are the parts that stop the same investigations being redone.
Current open work is in [`TODO.md`](TODO.md); the INVARIANTS and the
do-not-re-propose list were carried forward there rather than left here.

Tickets closed since then are appended at the end, under **Closed after the
archive**, as they close.
...
# CLOSED AFTER THE ARCHIVE

Tickets that closed in `TODO.md` after it was reopened on 2026-09-06, moved
here as they closed, in the order they closed. The text is as it stood in
`TODO.md`, with only the pointers that no longer make sense adjusted.
...
*(A cosmetic finding from this work — `--config` rejected before a
subcommand — is still open, and stayed in `TODO.md`.)*
...
From `private/guardian/guardian_report_20260905_213041.md`. The file passed: 0
CRITICAL, 0 HIGH, 0 MEDIUM. Its four informational findings are still in
`TODO.md`, where they guard against being re-filed; the two LOW ones closed.
...
### I-4 — the CA-bundle test exercised the empty path, not the decoders *(INFORMATIONAL)* ✅ DONE (2026-09-12)

`a_malformed_ca_bundle_is_rejected_not_ignored` actually exercises the *empty*
path: its input has no PEM markers, so it is caught by the `is_empty()` bail,
not by `from_pem_bundle`. The decode paths are pinned only by reqwest's
internals in-repo (the audit verified them externally). Renamed to
`a_ca_bundle_with_no_pem_sections_is_rejected`, and three decode-path tests
added: valid base64 that is not a certificate, a truncated real certificate,
and a good certificate followed by a corrupt one (which must fail whole —
silent truncation to the good half would install a trust set the operator's
...
## SUPERSEDED NON-ACTION

Was under RECORDED NON-ACTIONS in `TODO.md`. C-1 above reversed it on
2026-09-12: the https rejection now reports only the scheme, because the
userinfo gate could not be relied on to run first (a schemeless
`user:pass@host` parses with `user` as the scheme). Kept for the reasoning,
which was sound for the threat it considered and missed that one.
