...
- xtgeoip build -b # backup, as per manifest, then build using local copy of database
...
- xtgeoip build -b -f # force backup even without manifest, then build using local copy of database
- xtgeoip build -b -p # unsupported option, build has no prune function
...
- xtgeoip run -b # backup using manifest, then fetch, and build
...
- xtgeoip run -b -f # force backup even without manifest, then fetch, and build
- xtgeoip run -b -p # unsupported option, ambiguous (are we pruning bin or CSV?)
- xtgeoip run -b -p -f # unsupported option, ambiguous, cannot force prune (does force apply to backup or prune?)
- xtgeoip run -c # clean using manifest, then fetch, and build
- xtgeoip run -c -f # force clean even without manifest, then fetch, and build
