...
- ✅ **Man-page prose is checked as of 2026-09-03.** Five test-time checks: EXECUTION ORDER's four documented orderings against the real planner, plus both directions of the CONFIGURATION key set, the documented defaults against the shipped example, and the `[maxmind]` strictness claim against the parser. Teeth verified by reinstating each historical defect. A fourth defect surfaced in the process — `[logging] log_file` documented no default — and was fixed. Detail in `TODO.md`. ✅ **The generation-time half landed 2026-09-05** — `validate_manpage_template` in docgen: every option and command the template names must be one the spec declares, both directions for commands. It catches a rename or typo, not a false claim. Closing a real spec gap made it meaningful: `conf`'s `-d`/`-e`/`-s`/`-c` were declared nowhere in the spec, only in `cli.rs` and the template, and are now in `subcommand_options:`
- ✅ **The unknown-key asymmetry is closed (2026-09-05).** `deny_unknown_fields` now covers `Paths`, `Logging`, `Processing` and `Config` itself, not just `[maxmind]`. Every table rejects a key it does not define. The asymmetry was why `logging.verbose` sat in the shipped example being read by nothing — a key that looks live and does nothing cannot be told apart from one that works, from outside the program. **Breaking** for any config carrying an undefined key, which is the intent: such a config was already not doing what its author believed. The migration concern turned out to be narrower than feared — the shipped example was already clean, and is now asserted to parse under the strict rules by `shipped_example_config_parses`, which runs the real deserializer. The pre-existing `shipped_config_keys` only ever parsed into a `toml::Table`, so until now the example could have been rejected by the tool while every test passed. The man page's strictness claim was scoped to `[maxmind]` and is now stated generally
