...
- **Dependency updates are applied on the maintainer's terms, and the advisory check stays local.** Dependabot rejected 2026-09-04 — "always use the latest version" is a fallacy, and six crates are pinned exact for the credential path where a bump is a decision, not a chore. Do not propose it again
- **`cargo audit` gates both CI and `sync.py` (2026-09-03). No `schedule:` trigger — one was added and removed 2026-09-04; see `TODO.md` before re-adding.** Vulnerabilities block; `unsound`/`unmaintained`/`yanked` report. Do not tighten it to `-D warnings`: clippy's covers our code and is always fixable by us, while an advisory warning concerns a crate we do not own and may have no fix, so it would wedge every commit on an upstream's schedule. Do not judge the result by the exit code either — `cargo audit` exits 1 both for "vulnerable" and for "could not reach the database", so the gate branches on whether a parseable JSON report came back. Accepting a finding means an entry in `.cargo/audit.toml` carrying the ID, why it is unreachable here, and a review date
...
- **Nothing keeps Rust current on the dev machine.** `/usr/bin/{cargo,rustc}` are symlinks to `/usr/bin/rustup` (Ubuntu's apt `rustup` package); the toolchains live in `~/.rustup` and move *only* when `rustup update` is run by hand. No timer, no cron, no auto-update setting. That is why `stable` sat at 1.94.0 for six months. `sync.py` catches divergence from the pin, and `cargo audit` now covers dependency advisories (2026-09-03, below), but nothing yet reports that the pin itself or `rustup` (1.26.0 vs 1.29.1 upstream) has gone stale — see the maintenance question in `TODO.md`
- **Dependency advisories went unnoticed for four and a half months (2026-09-03).** The predicted staleness turned out to be a live exposure: `cargo audit` reported **six vulnerabilities**, the oldest published 2026-04-14. Cleared by `cargo update` — lockfile only, no `Cargo.toml` change — and a gate now exists so it cannot reopen silently. Detail in `TODO.md`
