...
fn validate_install(install: &InstallSpec) -> anyhow::Result<()> {
    ...
    // Every field below is interpolated into a tab-separated line by
    // `generate_install_manifest`, and a recipe splits that line on tabs. A
    // control character in any of them forges structure: a `\n` in `source`
    // ends the row early and starts a new one the consumer reads as a real
    // install entry, at a path and mode the manifest never declared.
    //
    // Checked here for *all* fields, not only the two that are otherwise
    // unconstrained. `producer` and `transform` are allowlisted and `mode` is
    // shape-checked, so today only `source` and `dest` can carry one — but
    ...
    let no_control = |field: &str, value: &str| -> anyhow::Result<()> {
        if let Some(c) = value.chars().find(|c| c.is_control()) {
            anyhow::bail!(
                "{field} {value:?} contains control character {c:?}; \
                 install-manifest.tsv is tab-separated and line-oriented, so \
                 it would forge a manifest row"
            );
        }
        Ok(())
    };
...
fn generate_cli_rules_rs(spec: &Spec) -> anyhow::Result<String> {
    ...
    for (const_name, reject, guards) in contexts {
        ...
        for g in lower_guards(reject, guards) {
            ...
            // `{:?}` for `key` for the same reason as error_text above: it
            // is spec text going into a Rust string literal, and a quote in
            // it would close the literal rather than appear in it.
            out.push_str(&format!(
...
fn generate_error_text_rs(spec: &Spec) -> anyhow::Result<String> {
    ...
    // `{:?}` rather than a raw string literal. `r#"{}"#` interpolates spec
    // text unescaped, so a `"#` in it closes the literal and everything after
    // is compiled as module-scope Rust — and the constant can still read as
    // the benign message, so a reader diffing the generated file for the text
    // they expected sees nothing wrong. Rust's Debug for `str` emits a quoted
    // literal with the escaping already done; the constant's *value* is
...
mod tests {
    ...
    /// A control character in any interpolated field must be refused, and
    /// refused twice.
    ///
    /// The manifest is tab-separated and line-oriented, so a `\n` in a field
    /// ends the row early and begins one the consumer reads as a real install
    /// entry — at a path and mode nothing declared. `contrib/README.md` ships
    /// the `while IFS=$'\t' read` loop that would act on it.
    ///
    /// Both layers are asserted separately, because they fail for different
    /// reasons and either could be removed without the other noticing:
    /// `validate_install` knows the field semantics,
    /// `generate_install_manifest` owns the format. A value can satisfy
    /// every semantic rule and still carry a delimiter, which is exactly
    /// how this got in.
    #[test]
    fn control_characters_cannot_forge_a_manifest_row() {
        let sound = |source: &str| InstallSpec {
            version: INSTALL_SCHEMA_VERSION,
            files: vec![InstallFile {
                source: source.to_string(),
                producer: "tracked".to_string(),
                transform: "none".to_string(),
                dest: "/usr/bin/xtgeoip".to_string(),
                mode: "0755".to_string(),
