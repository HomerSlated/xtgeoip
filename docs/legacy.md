...
## Switching from Legacy to Default Mode

If you previously ran `xtgeoip` with `--legacy` and later switch back to default mode, you will see this warning:

```
Warning: 2 orphaned files detected in "/usr/share/xt_geoip".
Warning: Please run `xtgeoip build -c -f` or manually delete files listed in "/usr/share/xt_geoip/orphaned" for a clean install.
```

The two orphaned files are `EU.iv4` and `EU.iv6`. These are listed in the `orphaned` file in your output directory. They are safe — and recommended — to remove:

- They are no longer generated by default mode.
- Their data overlaps with `O1.iv4` and `O1.iv6`, which is where default mode correctly places continent-only rows.
- Leaving them in place is harmless but wasteful, and may produce unexpected matches for the `EU` pseudo-code in xt_geoip rules.
...
This force-deletes all existing output files and rebuilds cleanly from the current archive, leaving no orphans.
